...
入力欄の「Pi Model」から利用可能なモデルを選択でき、現在の会話と履歴を維持します。変更はその会話に保存し、Pi全体の既定モデルは変更しません。新しい会話のモデルはPiの設定に従います。明示する場合は `nerita.pi.provider` と `nerita.pi.model` を両方指定し、再接続してください。
