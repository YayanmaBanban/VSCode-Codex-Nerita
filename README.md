...
Codexで最後に選択したモデルと推論レベルは、拡張機能のglobalState（`nerita.codex.lastModel`）へ保存し、次回の起動・新規会話で復元します。保存値は現在のモデル一覧で検証し、利用できない場合は対応する初期値へ戻します。履歴の再開・Forkでは履歴側の設定を優先します。Piの保存値とは独立しています。
...
入力欄の「Pi Model」から利用可能なモデルを選択でき、現在の会話と履歴を維持します。最後に選択したモデルと推論レベル（Ultraを含む）は拡張機能のglobalStateへ保存し、次回の新規会話・再起動時に復元します。保存値がなければPiの初期選択を使い、保存モデルが利用できない場合は利用可能な候補へフォールバックします。推論レベルが未対応になった場合は利用可能な値へ戻ります。履歴を再開する場合は履歴側の設定を使用します。

`pi install` で登録したパッケージを、Pi標準のユーザー設定と `.pi/settings.json` から読み込みます。導入・削除・更新はPi CLIで行い、新規会話または再接続で反映します。拡張の起動イベント、登録ツール、スキル、プロンプトテンプレートに対応します。スキルは入力欄の候補に表示し、`/skill:名前` と `/プロンプト名 引数` はSDKが展開します。登録ツールも操作ごとに承認します。Piのパッケージ解決に従って未取得の依存が取得される場合があります。
拡張はExtension Hostと同じ権限で動くため、起動イベントや拡張コード自体はツール承認による隔離対象ではありません。信頼するパッケージを使用してください。実行モードは `print`（`hasUI: false`）です。ターミナル専用UI・テーマ・キー操作はWebviewに適用しません。
